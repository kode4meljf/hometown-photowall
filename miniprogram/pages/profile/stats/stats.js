...
const { formatLikeCount, formatCompactNum } = require('../../../utils/util');
...
function formatDelta(n, period) {
  ...
  return { text: `+${formatCompactNum(num)} ${period}`, flat: false };
}

function mapDashboard(raw) {
  const p = raw.personal || {};
  const strip = [
    { key: 'works', num: formatCompactNum(p.works), lbl: '作品', ...formatDelta(p.deltaWorksMonth, '本月') },
    { key: 'likes', num: formatLikeCount(p.likes).text, lbl: '获赞', ...formatDelta(p.deltaLikesWeek, '本周') },
    { key: 'comments', num: formatCompactNum(p.comments), lbl: '评论', ...formatDelta(p.deltaCommentsWeek, '本周') },
    { key: 'views', num: formatCompactNum(p.views), lbl: '浏览', ...formatDelta(p.deltaViewsWeek, '本周') },
  ];
  ...
  if (raw.platform) {
    ...
    platform = {
      rows: [
        {
          name: '平台总作品',
          value: formatCompactNum(pl.totalPosts),
          ...formatDelta(pl.deltaPostsMonth, '本月'),
        },
        {
          name: '平台总获赞',
          value: formatCompactNum(pl.totalLikes),
          ...formatDelta(pl.deltaLikesMonth, '本月'),
        },
        {
          name: '平台总用户',
          value: formatCompactNum(pl.totalUsers),
